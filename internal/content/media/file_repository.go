...
func NewFileRepository(root string) (*FileRepository, error) {
	if strings.TrimSpace(root) == "" {
		return nil, errors.New("media repository path is required")
	}
...
func (r *FileRepository) ListCollections(context.Context) ([]Collection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read media repository: %w", err)
	}
...
func (r *FileRepository) readCollection(path string) (Collection, error) {
	...
	if err != nil {
		return Collection{}, fmt.Errorf("open media collection %q: %w", path, err)
	}
	defer file.Close()

	payload, err := io.ReadAll(file)
	if err != nil {
		return Collection{}, fmt.Errorf("read media collection %q: %w", path, err)
	}

	var collection Collection
	if err := json.Unmarshal(payload, &collection); err != nil {
		return Collection{}, fmt.Errorf("decode media collection %q: %w", path, err)
	}

	if strings.TrimSpace(collection.ID) == "" {
		return Collection{}, fmt.Errorf("media collection %q is missing id", path)
	}

	if strings.TrimSpace(collection.Title) == "" {
		return Collection{}, fmt.Errorf("media collection %q is missing title", path)
	}

	for _, item := range collection.Items {
		if strings.TrimSpace(item.ID) == "" {
			return Collection{}, fmt.Errorf("media collection %q contains item with missing id", path)
		}
		if strings.TrimSpace(item.Title) == "" {
			return Collection{}, fmt.Errorf("media collection %q contains item %q with missing title", path, item.ID)
		}
		if strings.TrimSpace(string(item.Kind)) == "" {
			return Collection{}, fmt.Errorf("media collection %q contains item %q with missing kind", path, item.ID)
		}
	}
